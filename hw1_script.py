...
if __name__ == "__main__":
    ...
    print("d ------------------------------------\n")
    im = Images_gray['Ricefields_gray']
    tm = np.arange(256)
    ...
    nim, _a, _b = contrastEnhance(Images_gray['Lighthouse_gray'], [0, 255])
    TMim, TM = SLTmap(Images_gray['Lighthouse_gray'], nim)
    d = np.sum(nim - np.matmul(sliceMat(Images_gray['Lighthouse_gray']), TM.transpose()).reshape(len(nim), len(nim[0]))) # computationally compare
    print("sum of diff between image and slices*[0..255] = {}".format(d))

    # then display
    plt.figure()
    plt.subplot(1, 2, 1)
    plt.imshow(Images['Lighthouse'])
    ...
    plt.figure()
    plt.subplot(1, 2, 1)
    plt.imshow(Images_gray['Binary1_gray'], cmap='gray', vmin=0, vmax=255)
    plt.title("The Image")
    plt.subplot(1, 2, 2)
    plt.imshow(negative_im, cmap='gray', vmin=0, vmax=255)
    plt.title("The negative image using SLT")
